...
assert(page.includes("JobPostStructuredDetails") && page.includes("commune-job-facts-grid") && page.includes("commune-job-narrative-grid"), "Job Post public detail must separate compact facts from wide narrative context.");
assert(page.includes("!jobPost || !canManageListingStatus") && page.includes("isModerator || (userId && post.viewer_is_owner)"), "Job Post operator controls must remain limited to the author or assigned reviewers rather than anonymous readers.");
assert(page.includes("commune-post-context-header") && page.includes("!postId && <>") && page.includes("!postId && !isLobby"), "individual Commune post routes must use compact context instead of repeating landing-page hero/doctrine/account panels.");
assert(page.includes("jobApplicationRoutePublicLabel") && jobOpportunityModel.includes("Private Work With Elysia Ecobotics"), "public Job Post detail must use reader-facing Private Work With language without changing its stored route.");
for (const v2BodyHeading of ["Opportunity type", "Compensation status", "Application destination", "Future-role notice"]) {
  assert(page.slice(page.indexOf('job_post: ['), page.indexOf('research_note: [')).includes(`"${v2BodyHeading}"`), `Job Post body suppression is missing v2 heading: ${v2BodyHeading}`);
}
...
assert(hostedAllowancePage.includes("Local Elysia is unaffected") && hostedAllowancePage.includes("Additional paid allowance") && /No paid top-up is offered/i.test(hostedAllowancePage), "The allowance page must preserve the local/hosted/payment boundary.");
