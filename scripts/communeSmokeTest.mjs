...
const jobPostWorkflowMigration = await read("supabase/migrations/2026_06_26_job_post_structured_workflow.sql");
...
assert(signalConsolePage.includes("Job Post activity") && signalConsolePage.includes("My Job Posts") && signalConsolePage.includes("Listings needing admin approval or anti-scam follow-up"), "Signal Console should render Job Post activity/review sections.");
assert(signalConsolePage.includes("Job Post signals track public opportunity listings") && signalConsolePage.includes("Work With remains the private application/intake path"), "Signal Console should preserve Job Post public-board/private-intake boundary copy.");
...
assert(softDeleteCleanupMigration.includes("soft_delete_commune_post(target_post_id uuid, moderation_note text default null)") && accountApi.includes('label: "target_post_id/moderation_note"') && accountApi.includes('label: "p_target_post_id/p_moderation_note"'), "Commune post delete RPC should document the SQL argument names and tolerate PostgREST p-prefixed signature drift.");
assert(accountApi.includes("isSoftDeleteRpcSignatureError") && accountApi.includes("softDeleteRpcErrorMessage") && page.includes("setStatus(visibleMessage)") && page.includes('<p className="message">{status}</p>'), "Commune admin delete errors should surface safe RPC failure details in the Admin Moderation panel.");
...
assert(softDeleteCleanupMigration.includes("update public.commune_vote_posts") && softDeleteCleanupMigration.includes("vote_rows_archived"), "Commune soft-delete cleanup should archive/suppress Community Voting Room sidecars after parent delete.");
...
assert(page.includes("locallyDeletedPostId === postId ? null : state.posts[0]") && page.includes("onDeleted={setLocallyDeletedPostId}"), "CommunityVoteDetail should not keep rendering from stale local state after Admin Moderation delete succeeds.");
