...
assert(marketplaceDetails.includes("Install intent unavailable"), "Marketplace details must block install intent for candidate/unavailable listings.");
...
assert(marketplaceSeeds.includes("Codev") && marketplaceSeeds.includes('status: "pending_review"') && marketplaceSeeds.includes('listing_stage: "official_candidate"'), "Codev must remain a truthful non-installable official candidate.");
...
assert(marketplaceSubmission.includes("Submit private pending review") && marketplaceSubmission.includes("will leave my computer") && marketplaceSubmission.includes("Git repository URL (metadata only)"), "Marketplace Submit must expose governed source intake, upload disclosure, and non-fetching Git metadata.");
assert(addonIntakePanel.includes("Folder or repository") && addonIntakePanel.includes("Package or source bundle") && addonIntakePanel.includes("does not upload them"), "Browser add-on intake paths or local-only selection truth are missing.");
