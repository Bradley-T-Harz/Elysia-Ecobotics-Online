...
const commonsAppearance = await read("src/pages/The-Commons-Circle/ProfileAppearanceSettingsPage.tsx");
...
const badgeAdminClient = await read("src/shared/review/badgeAdminClient.ts");
...
const archiveReleaseManifest = await read("src/pages/The-Elysia-Archive/releaseManifest.json");
const publicReleaseManifest = await read("public/release-manifest.json");
...
const marketplaceSubmission = await read("src/pages/The-Elysia-Marketplace/components/DeveloperSubmissionForm.tsx");
...
const listingTruth = await read("src/pages/The-Elysia-Marketplace/lib/listingTruth.ts");
const manifestApi = await read("src/pages/The-Elysia-Marketplace/pages/ManifestApiPage.tsx");
const webManifest = await read("public/manifest.webmanifest");
...
assert(commonsAppearance.includes("COMMONS_THEME_MODES.map") && commonsAppearance.includes("theme.label") && commonsAppearance.includes("normalizeCommonsThemeMode(draft.theme_mode)"), "Profile & Appearance Theme mode dropdown should use shared readable labels and normalized values.");
assert(!commonsAppearance.includes("themeModes.map") && !commonsAppearance.includes(">{theme}</option>"), "Profile & Appearance Theme mode dropdown should not render raw theme keys as option labels.");
assert(publicProfile.includes("getCommonsThemeModeOption(themeMode).label") && commonsAppearance.includes("getCommonsThemeModeOption(draft.theme_mode).label"), "Public and settings-preview theme chips should use readable theme mode labels.");
...
assert(commonsAppearance.includes("COMMONS_DECORATIVE_MARKER_SET_OPTIONS.map") && commonsAppearance.includes("option.label") && !commonsAppearance.includes(">{decal}</option>"), "Profile & Appearance decorative marker dropdown should use shared readable labels, not raw keys.");
assert(!commonsAppearance.includes("Selected decorative markers") && !commonsAppearance.includes("commons-decal-picker") && !commonsAppearance.includes("toggleSelectedDecal") && !commonsAppearance.includes("COMMONS_DECORATIVE_MARKERS.map") && !commonsAppearance.includes("selected_decals).includes"), "Profile & Appearance should not render deprecated selected decorative marker checkbox controls.");
...
assert(!commonsAppearance.includes("commons-public-profile-hero-marker") && !commonsAppearance.includes("commons-public-decorative-marker-layer"), "Customization Studio preview must not render decorative marker PNG overlays.");
...
assert(commonsAppearance.includes("COMMONS_PROFILE_LAYOUTS.map") && commonsAppearance.includes("option.label") && commonsAppearance.includes("normalizeCommonsProfileLayout(draft.profile_layout)"), "Profile & Appearance layout dropdown should use shared labels and normalized values.");
assert(commonsAppearance.includes("CommonsProfileLayoutFrame") && commonsAppearance.includes('variant="preview"') && commonsAppearance.includes("getCommonsProfileLayoutOption(draft.profile_layout).label"), "Profile & Appearance preview should use the shared profile layout frame and readable layout labels.");
...
assert(!badgeAdminClient.includes("Administrators cannot grant badges to themselves") && !badgeAdminClient.includes("Administrators cannot restore their own badges"), "The badge client must not reintroduce a blanket administrator self-target refusal.");
assert(adminPage.includes("including their own signed-in account") && adminPage.includes("Use my signed-in account") && adminPage.includes("Badge management requires administrator authority."), "Badge management must support audited administrator self-targeting while retaining the administrator gate.");
...
assert(reviewClient.includes("loadStewardshipReceiptEvidence") && reviewClient.includes('createSignedUrl(file.storage_path, expiresInSeconds, { download: "redacted-proof" })') && reviewClient.includes("expiresInSeconds = 300"), "Stewardship reviewers need a bounded private-proof access path.");
assert(reviewClient.includes("proofStorageMetadataMatches(file, item.source_id, request.user_id)") && reviewClient.includes("Private proof metadata failed its storage-boundary check"), "Private stewardship proof access must verify its exact storage boundary before signing.");
...
assert(adminPage.includes("Private stewardship proof") && adminPage.includes("Download private proof (5-minute link)") && adminPage.includes("the link is private and expires"), "Admin Stewardship Review must provide a short-lived reviewer-only proof path without rendering storage paths.");
...
assert(commonsSetup.includes("Confirm receipt/proof redaction") && commonsSetup.includes("private to you and authorized stewardship reviewers") && commonsSetup.includes('status: "pending_review"'), "Stewardship proof intake must require redaction acknowledgement and remain pending scoped private review.");
assert(workWithPage.includes("private to you and authorized Work With reviewers") && !workWithPage.includes("administrator-review materials") && !commonsSetup.includes("administrator-review materials"), "Private upload notices must reflect scoped review authority.");
...
assert(commonsApi.includes("shortPublicBio") && commonsApi.includes("canonicalProfileUrl") && commonsApi.includes("decodeFeaturedPublicLinks"), "Public Commons identity and legacy public fields should be decoded only from the bounded visibility-aware presentation.");
...
assert(publicProfileLoader.includes("presentation.publicBadges") && !publicProfileLoader.includes('from("visible_user_badges")') && !publicProfileLoader.includes('from("user_badges")'), "Public Commons profiles must consume badge presentation from the bounded server projection without account-keyed follow-up reads.");
...
assert(commonsAppearance.includes("Shape your public profile room") && commonsSetup.includes("Organization, optional"), "Account settings should preserve public profile customization and editing paths.");
...
assert(commonsSetup.includes('placeholder="Choose a public username"') && commonsSetup.includes('placeholder="How you\'d like to appear"'), "New Commons Profile identity fields must use neutral user-choice placeholders.");
assert(!commonsSetup.includes('placeholder="bradley-harz"') && !commonsSetup.includes('placeholder="Bradley T. Harz"'), "Commons Profile setup must not contain Bradley-specific identity placeholders.");
assert(commonsSetup.includes("Boolean(profile?.commons_onboarding_completed_at)") && commonsSetup.includes("setupComplete && loaded") && commonsSetup.includes("accountSessionKey(sessionKeys.profileDraft, userId)"), "Commons Profile setup must distinguish completed saved profiles from incomplete compatibility rows and scope drafts to the authenticated account.");
assert(!commonsSetup.includes("user_metadata") && !commonsSetup.includes("full_name") && !commonsSetup.includes("auth.user.email"), "Commons Profile setup must not seed public fields from private authentication identity metadata.");
...
assert(commonsApi.includes("ownerProfileMediaCache") && commonsApi.includes("signedAt") && commonsApi.includes("expiresAt") && commonsApi.includes("ownerProfileMediaInFlight"), "Private owner-avatar previews must use an identity-bound, expiry-aware, single-flight cache.");
assert(commonsApi.includes("renewOwnerProfileMediaPreview") && commons.includes("renewOwnerAvatar") && commons.includes("imageIdentity={savedCustomization.avatar_media_id}"), "Commons Homebase must support one canonical avatar renewal without altering the public proxy path.");
assert(commonsAvatarViewer.includes("failedSrc") && commonsAvatarViewer.includes("onImageError") && commonsAvatarViewer.includes("renewalRef"), "Commons avatar rendering must replace failed nonempty URLs with a stable fallback and bound renewal by media identity.");
...
assert(commonsAppearance.includes("Remove profile picture") && commonsAppearance.includes("Remove banner") && commonsAppearance.includes("Avatar and banner media update when selected"), "Profile & Appearance should expose public avatar/banner removal and boundary copy.");
assert(commonsAppearance.includes("mediaStatus") && commonsAppearance.includes("Uploading") && commonsAppearance.includes("aria-live=\"polite\""), "Profile & Appearance should show immediate avatar/banner upload status without relying on Save customization.");
assert(commonsAppearance.includes("Unsaved preview. Save to publish these style choices.") && commonsAppearance.includes("Saved customization is live. New changes preview here first."), "Profile & Appearance should distinguish unsaved preview from saved public profile styling.");
...
assert(commonsAppearance.includes("Banner framing") && commonsAppearance.includes("Banner zoom") && commonsAppearance.includes("Horizontal position") && commonsAppearance.includes("Vertical position") && commonsAppearance.includes("Reset banner framing"), "Profile & Appearance should expose Banner framing controls.");
assert(commonsAppearance.includes("function signature") && commonsAppearance.includes("banner_zoom: normalizeCommonsBannerZoom(settings.banner_zoom)") && commonsAppearance.includes("banner_position_x: normalizeCommonsBannerPosition(settings.banner_position_x)") && commonsAppearance.includes("banner_position_y: normalizeCommonsBannerPosition(settings.banner_position_y)"), "Profile & Appearance unsaved-change detection should include banner framing values.");
assert(commonsAppearance.includes("const previewStyle = commonsCustomizationStyle(draft)") && commonsAppearance.includes("style={previewStyle}"), "Profile & Appearance preview should apply draft banner/theme/accent CSS variables.");
...
assert(commonsAppearance.includes("Avatar and banner media update when selected") && commonsAppearance.includes("Decorative markers are public visual labels"), "Profile & Appearance should honestly distinguish immediate media uploads from limited decorative markers.");
assert(commons.includes("customizationSkinClass(savedCustomization)") && commonsAppearance.includes("customizationClass(draft)") && commonsAppearance.includes("commons-customization-preview"), "Commons Circle live homebase should use saved skin customization while settings drafts render in a bounded layout preview.");
...
assert(commonsAppearance.includes("commons-circle-customization-preview-mantle") && commonsAppearance.includes("commons-circle-customization-preview-banner-image"), "Profile & Appearance preview should expose private-only banner fit hooks.");
...
assert(commonsAppearance.includes("COMMONS_BACKGROUND_STYLES.map") && commonsAppearance.includes("style.label") && commonsAppearance.includes("normalizeCommonsBackgroundStyle(draft.background_style)"), "Profile & Appearance Background style dropdown should use shared labels and normalized values.");
assert(commonsAppearance.includes("CommonsBackgroundAtmosphere") && commonsAppearance.includes('variant="preview"') && commonsAppearance.includes("getCommonsBackgroundStyleOption(draft.background_style).label"), "Profile & Appearance preview should use the shared atmosphere component and readable background labels.");
assert(commonsAppearance.includes("Revert preview") && commonsAppearance.includes("setDraft(saved)"), "Profile & Appearance should let users discard unsaved preview changes.");
...
const publisherMigration = await read("supabase/migrations/20260909020000_marketplace_publisher_ownership.sql");
assert(publisherMigration.includes("publisher_lock_submitted_draft after insert") && publisherMigration.includes("locked_reason='submitted_for_marketplace_review'") && forgeApi.includes("submitDraftForReview"), "Submission must atomically lock the draft in PostgreSQL with an explicit reason.");
...
assert(!supportBilling.includes("createSandboxCreditCheckout") && !supportBilling.includes("loadSandboxCreditCatalog") && !supportBilling.includes("No option is preselected, no automatic purchase occurs"), "Support & Billing must not expose the dormant paid hosted-execution checkout while payment activation remains externally gated.");
assert(supportBilling.includes("Allowance has its own account page") && supportBilling.includes('/commons-circle/settings/hosted-execution') && supportBilling.includes("Additional paid hosted execution is unavailable pending the separate payment-system decision"), "Support & Billing must direct hosted allowance to its Settings-owned truth surface and state that paid allowance is unavailable.");
assert(checkoutReturnStatus.includes("Browser return alone is not proof of payment or fulfillment") && checkoutReturnStatus.includes("loadBillingOrder(orderReference, accessToken)") && checkoutReturnStatus.includes("order?.flow === expectedFlow") && checkoutReturnStatus.includes("The private order belongs to a different economic flow"), "Dormant economic checkout returns must retain private server verification and exact-flow refusal for any later separately approved activation.");
...
assert(commonsApi.includes("visibleSavedCommuneRows") && commonsApi.includes("filterNotificationsByActiveCommunePost") && commonsApi.includes("decodePublicCommuneComments"), "Deleted/removed Commune posts should not remain in Saved Shelves, Signal Console, Homebase notifications, or bounded public-profile contribution cards.");
