import './wordingCleanupLegalCheck.mts';
...
const pages=Object.fromEntries(legalPolicyPages.filter(p=>ownerDecisionLegalSlugs.has(p.slug)).map(p=>getLegalPolicy(p.slug,version)!).map(p=>[p.slug,{slug:p.slug,route:p.route,title:p.title,status:p.status,lastUpdated:p.lastUpdated,version,contentSha256:crypto.createHash('sha256').update(serializeEconomicLegalSemanticContent(p)).digest('hex')}]));
...
// Check the immutable owner-decision revision, including after a later public wording revision.
assert.deepEqual(saved,{pages,bundles},'Frozen owner-decision policy hashes must match the exact reviewed content.');
...
console.log('Owner policy legal integrity passed: 14 frozen owner-decision versions, 8 staged bundles, frozen historical pages.');
