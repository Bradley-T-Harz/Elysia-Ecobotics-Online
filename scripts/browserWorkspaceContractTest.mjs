...
try {
  ...
  const forms = new BrowserWorkspace(owner, 'Forms', initial(), undefined, { creator: 'Fixture', activeFile: 'LICENSE' });
  assert.equal(forms.getSnapshot().revision, 0);
  assert.equal(forms.getSnapshot().dirty, false);
  forms.updateMetadata({ creator: 'Edited fixture', activeFile: 'LICENSE' });
  assert.equal(forms.getSnapshot().revision, 1);
  const formsRecovery = await forms.recovery();
  const formsRestored = await BrowserWorkspace.restore(owner, formsRecovery);
  assert.deepEqual(formsRestored.getSnapshot().metadata, forms.getSnapshot().metadata);
  for (const metadata of [{ consent: true }, { nested: { accessToken: 'fixture' } }, { grant: [] }, { termsAccepted: true }]) assert.throws(() => forms.updateMetadata(metadata), /Authorization/);
  forms.importFiles([{ path:'src/replacement.py', text:'answer = 5', provenance:'intake' }], forms.getSnapshot().revision, true);
  assert.deepEqual(forms.getSnapshot().files.map(file=>file.path), ['src/replacement.py'], 'complete import replaces the previous identity and source');
  await assert.rejects(forms.preparePackage('missing-manifest'), /manifest/);
  forms.importFiles([{path:'manifest.json',text:manifest,provenance:'intake'}]);
  assert.equal(forms.getSnapshot().files.length, 2, 'explicit manifest-only import retains the chosen source');
