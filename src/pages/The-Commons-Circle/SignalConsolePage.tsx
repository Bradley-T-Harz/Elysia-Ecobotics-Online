...
export default function SignalConsolePage() {
  ...
  return <div className="page-stack commons-circle-page commons-signal-console">
    ...
    {data?.signedIn && <section className="section-card">
      <div className="section-heading"><p className="eyebrow">Compatibility migration map</p><h2>Signals remains available while each destination proves parity</h2><p>This console still preserves its direct-source author, reviewer, administrator, domain, and sandbox views. The newer account rooms separate actions, informational outcomes, and your submitted workflows without changing source authority.</p></div>
      <div className="commons-homebase-grid">
        <article className="section-card commons-account-room-card"><p className="eyebrow">Inbox</p><h3>Private actions and conversations</h3><p>Open source-backed actions, conversation requests, and governed private communication.</p><Link className="button-link button-link--primary" to="/commons-circle/inbox">Open Inbox</Link></article>
        <article className="section-card commons-account-room-card"><p className="eyebrow">Notifications</p><h3>Informational events and outcomes</h3><p>Read, archive, and filter recipient-scoped notifications with exact unread counts.</p><Link className="button-link button-link--primary" to="/commons-circle/notifications">Open Notifications</Link></article>
        <article className="section-card commons-account-room-card"><p className="eyebrow">Requests &amp; Reviews</p><h3>Your submitted source workflows</h3><p>Track proposals, Work With requests, Job Posts, Research Notes, showcases, and their authoritative status.</p><Link className="button-link button-link--primary" to="/commons-circle/requests-reviews">Open Requests &amp; Reviews</Link></article>
        {data.canOpenReviewCenter && <article className="section-card commons-account-room-card"><p className="eyebrow">Authorized staff</p><h3>Review Center</h3><p>Specialist reviewer, moderator, steward, anti-scam, and administrator queues remain role-gated.</p><Link className="button-link button-link--primary" to="/admin/review">Open Review Center</Link></article>}
      </div>
      <p className="boundary-note">This compatibility route is not redirected yet. Existing bookmarks, query strings, authentication returns, proposal links, and every legacy section below remain available during reconciliation and production observation.</p>
    </section>}
    {data?.signedIn && <section className="section-card">
      ...
      {data.canReviewCommune && <><div className="section-heading"><p className="eyebrow">Troubleshooting Grove review</p><h3>Issues needing review or follow-up</h3><p>Authorized Commune reviewers can see needs-information, in-progress, and fix-proposed troubleshooting records while original authors retain control over accepted fixes.</p></div>
      ...
      {data.canReviewCommune && <><div className="section-heading"><p className="eyebrow">Research Notes review</p><h3>Notes needing citation or clarification</h3><p>Authorized Commune reviewers can see submitted, needs-citation, source-issue, and overclaiming-evidence states without exposing hidden moderator notes publicly.</p></div>
      ...
      {data.canReviewCommune && <><div className="section-heading"><p className="eyebrow">Repository Showcase review</p><h3>Repository showcases needing review</h3><p>Authorized Commune reviewers can see pending, in-review, and needs-information Repository Showcase rows without treating them as safe, compatible, licensed, or installable.</p></div>
      ...
      {data.canReviewCommune && <><div className="section-heading"><p className="eyebrow">Elysia Iteration Showcase review</p><h3>Iterations needing review</h3><p>Authorized Commune reviewers can see pending, in-review, and needs-information iteration metadata without treating it as official, compatible, installable, or Marketplace-ready.</p></div>
      ...
      {data.canReviewCommune && <><div className="section-heading"><p className="eyebrow">Job Post review</p><h3>Listings needing admin approval or anti-scam follow-up</h3><p>Authorized Commune reviewers can see not-reviewed, suspicious, and needs-clarification Job Post rows without exposing hidden admin notes publicly.</p></div>
      ...
      {data.canReviewCommune && <><div className="section-heading"><p className="eyebrow">Community Voting Room activity</p><h3>My Community Voting Room votes</h3><p>Community Voting Room signals stay separate from Official Update. Admin-created votes guide stewardship decisions and do not automatically govern the site.</p></div>
