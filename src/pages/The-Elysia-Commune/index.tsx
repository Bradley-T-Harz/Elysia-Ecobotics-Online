  ...
  return <section className="commune-proposal-workspace">
    ...
    <div className="commune-proposal-grid">
      ...
      {selectedProposal && <article className="commune-proposal-detail">
        ...
        <div className="button-row">{canDecide && <><button className="button-primary" type="button" onClick={() => void decideProposal("accepted")}>{isTroubleshootingWorkbench ? "Accept fix" : "Accept revision"}</button><button type="button" onClick={() => void decideProposal("needs_changes")}>Ask for changes</button><button type="button" onClick={() => void decideProposal("rejected")}>{isTroubleshootingWorkbench ? "Reject fix" : "Reject revision"}</button></>}{canWithdraw && <button type="button" onClick={() => void withdrawProposal()}>Withdraw my proposal</button>}{canMessageProposalParticipant && <Link className="button-link" to={`/commons-circle/inbox?view=messages&sourceDomain=code_proposals&sourceType=commune_code_revision_proposal&sourceRecord=${selectedProposal.id}`}>Message proposal participant</Link>}{account.isModerator && ["submitted", "needs_changes"].includes(selectedProposal.proposal_status) && <button type="button" onClick={() => void decideProposal("hidden_by_moderation")}>Hide unsafe proposal</button>}</div>
