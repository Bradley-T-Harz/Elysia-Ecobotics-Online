...
import { CodevBindingProvider, CodevWorkspaceBindingRegistration, type CodevWorkspaceBinding } from "../../shared/codev/workspaceBinding";
import { consumeForgeRefreshView, storeForgeRefreshView, type ForgeRefreshView } from "../../shared/codev/forgeRefreshView";
...
function DraftWorkspace({ draft, catalog, onChanged, refreshView }: { draft: AddonDraft; catalog: PermissionDefinition[]; onChanged: (messages: string[]) => void; refreshView?: ForgeRefreshView | null }) {
  const auth = useAuth();
  ...
  const [activeFilePath, setActiveFilePath] = useState(refreshView?.activeFilePath ?? "manifest.json");
  ...
  async function run(action: () => Promise<unknown>) { pendingOperations.current++; try { await action(); } catch (error) { report([error instanceof Error ? error.message : "The workspace operation could not be confirmed."]); } finally { pendingOperations.current--; } }
  const codevPolicy = useRef({ readOnly, activeFilePath, results, parsed, intake, onChanged });
  codevPolicy.current = { readOnly, activeFilePath, results, parsed, intake, onChanged };
  const codevBinding = useMemo<CodevWorkspaceBinding>(() => ({ controller,
    canEdit: () => alive.current && !codevPolicy.current.readOnly && pendingOperations.current === 0,
    activeFilePath: () => {
      const files = controller.model.getSnapshot().files;
      return files.some(file => file.path === codevPolicy.current.activeFilePath) ? codevPolicy.current.activeFilePath : files[0]?.path ?? "";
    },
    sourceDescription: () => "Browser Forge draft: " + (codevPolicy.current.intake?.sourceKind ?? "virtual files"),
    diagnostics: () => [...codevPolicy.current.parsed.results, ...codevPolicy.current.results].map(result => `${result.severity}: ${result.message}`),
    beforeRefresh: async () => {
      if (!alive.current || pendingOperations.current) throw new Error("Wait for the current Forge operation before refreshing.");
      const revision = await controller.persist();
      if (!alive.current || pendingOperations.current) throw new Error("The Forge operation changed during recovery. Your files remain open.");
      controller.model.assertRevision(revision);
    },
    onChanged: message => { if (alive.current) codevPolicy.current.onChanged([message]); }
  }), [controller]);
...
export default function DeveloperForgePage() {
  const auth = useAuth();
  const [codevBinding, setCodevBinding] = useState<CodevWorkspaceBinding | null>(null);
  const browserId = browserWorkspaceId();
  const refreshView = useMemo(() => consumeForgeRefreshView(auth.userId, browserId), [auth.userId, browserId]);
  const pendingCreation = useRef(0);
  ...
  const selectedDraft = draftId ? state?.drafts.find((draft) => draft.id === draftId) : state?.drafts.find(draft => draft.id === refreshView?.selectedDraftId) ?? state?.drafts[0];
  ...
  useEffect(() => { setOwnership(refreshView?.ownership ?? emptyOwnership); setBlankName(refreshView?.blankName ?? "Example Add-on"); setMessages([]); }, [auth.userId, refreshView]);
  const restoredScroll = useRef<string | null>(null);
  useLayoutEffect(() => {
    if (!refreshView || !state || refreshView.route !== route || restoredScroll.current === route) return;
    restoredScroll.current = route;
    const frame = requestAnimationFrame(() => { requestAnimationFrame(() => window.scrollTo({ top: refreshView.scrollY, behavior: "instant" })); });
    return () => cancelAnimationFrame(frame);
  }, [refreshView, state, route]);
  function preserveCodevView() {
    if (!auth.userId || activeAccount.current !== auth.userId || pendingCreation.current) throw new Error("Wait for the current Forge action before refreshing.");
    storeForgeRefreshView(auth.userId, browserId, { route, selectedDraftId: codevBinding?.controller.model.getSnapshot().owner.draftId ?? null,
      activeFilePath: codevBinding?.activeFilePath?.() || null, blankName, ownership, scrollY: window.scrollY });
  }
