...
function TemplateGallery({ profileId, onCreated, ownership }: { profileId?: string | null; onCreated: (messages: string[]) => void; ownership?: OwnershipSelection }) {
  ...
  async function create(manifest: ForgeManifest) {
    const result = await createDraftFromManifest(manifest, profileId, ownership ?? selection);
...
export default function DeveloperForgePage() {
  ...
  const [stateRecord, setStateRecord] = useState<{ token: string | null; state: ForgeState } | null>(null);
  const state = stateRecord?.token === auth.accessToken ? stateRecord.state : null;
  ...
  const refresh = async () => {
    ...
    if (next.userId === auth.userId) setStateRecord({ token: auth.accessToken, state: next });
    if (import.meta.env.DEV && next.warnings.length) console.warn("[Developer Forge]", next.warnings);
  };
  useEffect(() => { setOwnership(emptyOwnership); void refresh(); }, [auth.userId, auth.accessToken]);
  ...
  async function createBlank() {
    ...
    const result = await createDraftFromManifest(manifest, state?.profile?.id ?? null, ownership);
