...
function DraftWorkspace({ draft, catalog, onChanged }: { draft: AddonDraft; catalog: PermissionDefinition[]; onChanged: (messages: string[]) => void }) {
  ...
  async function exportCurrentDraftPackage() {
    ...
    const blob = await buildManifestPackage({ id: working.addon_slug || "developer-forge-draft", name: working.addon_name || "Developer Forge Draft", manifest, readme: `# ${working.addon_name}\n\n${working.long_description || working.short_summary}\n\nThis is an inert Developer Forge export. The public website did not execute, install, build, or approve this add-on.`, license: working.license, changelog: "# Changelog\n\n## Draft\n- Exported from Developer Forge as inert package materials." });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
