...
export default function SupportThankYouPage() {
  ...
  return <div className="page-stack support-page support-thank-you-page">
    ...
    <section className="section-card support-order-state" aria-live="polite" aria-busy={loading}>
      ...
      {order.status === "verified" && supportFlowMatches && <p className="inline-status">A payment record does not prove email delivery. Stripe can send a receipt when the relevant delivery settings are enabled; retain the provider confirmation and use Support & Billing for help. Signed-in support will appear in Support &amp; Billing after private reconciliation completes.</p>}
