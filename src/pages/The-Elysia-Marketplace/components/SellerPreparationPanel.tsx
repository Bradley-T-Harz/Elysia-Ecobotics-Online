  ...
  return <section className="preparation-workspace" aria-labelledby="seller-preparation-title">
    ...
    {state && <>
      ...
      {seller && !["suspended", "withdrawn", "rejected"].includes(seller.status) && <PreparationForm key={editing?.offerId ?? offerId} title={editing ? "Revise offering preparation" : "Prepare an offering"} busy={disabled} label="Save offering draft" submit={async data => {
        ...
        {kind === "commercial" && <><label><span>Proposed price in USD cents</span><input name="amountMinor" type="number" defaultValue={editing?.amountMinor ?? ""} min={1} max={100_000_000_000} step={1} required /></label><label><span>Adopted platform fee reference</span><select name="feeProposalId" defaultValue={editing?.feeProposalId ?? "e9090800-0000-4000-8000-000000000005"} required>{state.proposals.filter(proposal => proposal.kind === "marketplace_fee_bps" && proposal.value === 500).map(proposal => <option key={proposal.proposalId} value={proposal.proposalId}>{proposal.value / 100}% · adopted owner policy · historical reference  {new Date(proposal.createdAt).toLocaleDateString()}</option>)}</select></label><p>The adopted platform fee is 5% on successful paid third-party sales. Free add-ons carry no fee, and no separate buyer convenience fee applies. Provider approval remains pending; this draft is not a purchasable offer.</p></>}
