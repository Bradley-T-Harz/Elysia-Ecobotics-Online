...
export type StewardshipReceiptEvidence = {
  displayName: string;
  mimeType: string;
...
export async function loadStewardshipReceiptEvidence(
  item: ReviewItem
): Promise<{ evidence: StewardshipReceiptEvidence | null; warning?: string }> {
  if (!hasSupabaseConfig || !supabase) return { evidence: null, warning: supabaseNotConfiguredMessage };
  if (item.domain !== "stewardship" || item.source_table !== "stewardship_recognition_requests") {
    return { evidence: null, warning: "Private stewardship proof is not available for this review item." };
  }
  ...
  return {
    evidence: {
      ...
      mimeType: String(file.mime_type ?? "application/octet-stream").slice(0, 100),
      sizeBytes: Number.isFinite(file.size_bytes) && Number(file.size_bytes) >= 0 ? Number(file.size_bytes) : null,
      sha256Prefix: /^[a-f0-9]{64}$/.test(normalizedHash) ? normalizedHash.slice(0, 12) : null,
      redactionStatus: String(file.redaction_status ?? "not_recorded").slice(0, 80),
