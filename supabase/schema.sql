...
-- Snapshot append: marketplace publish/revoke pipeline (2026_06_13).
...
-- Snapshot append: add-on archive inspection metadata (2026_06_14).
...
-- supabase/legacy-migrations/2026_06_26_official_update_structured_workflow.sql
...
-- supabase/legacy-migrations/2026_06_26_research_notes_structured_workflow.sql
...
-- Job Post structured workflow snapshot. Canonical repair migration:
-- supabase/legacy-migrations/2026_06_26_job_post_structured_workflow.sql
...
create policy "public reads published job post metadata" on public.commune_job_posts
  for select to anon, authenticated
  using (exists (select 1 from public.commune_posts p where p.id = post_id and p.post_type = 'job_post' and p.status = 'published' and p.visibility = 'public') or author_user_id = auth.uid() or public.current_user_can_review_domain('commune'::public.review_domain));
...
create policy "signed users create own job post metadata" on public.commune_job_posts
  for insert to authenticated
  with check (author_user_id = auth.uid() and exists (select 1 from public.commune_posts p where p.id = post_id and p.user_id = auth.uid() and p.post_type = 'job_post' and p.status in ('pending_review','published') and (p.status <> 'published' or public.current_user_is_admin())));
...
create or replace function public.update_own_commune_job_post_application_status(p_job_post_id uuid default null, p_post_id uuid default null, p_application_status text default 'open', p_public_correction_note text default null)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select * into v_row from public.commune_job_posts where (p_job_post_id is not null and id = p_job_post_id) or (p_job_post_id is null and p_post_id is not null and post_id = p_post_id) limit 1;
...
create or replace function public.touch_commune_vote_posts_updated_at() returns trigger language plpgsql as $$ begin new.updated_at = now(); return new; end; $$;
drop trigger if exists touch_commune_vote_posts_updated_at on public.commune_vote_posts;
create trigger touch_commune_vote_posts_updated_at before update on public.commune_vote_posts for each row execute function public.touch_commune_vote_posts_updated_at();
create or replace function public.touch_commune_vote_options_updated_at() returns trigger language plpgsql as $$ begin new.updated_at = now(); return new; end; $$;
drop trigger if exists touch_commune_vote_options_updated_at on public.commune_vote_options;
create trigger touch_commune_vote_options_updated_at before update on public.commune_vote_options for each row execute function public.touch_commune_vote_options_updated_at();
create or replace function public.touch_commune_vote_ballots_updated_at() returns trigger language plpgsql as $$ begin new.updated_at = now(); return new; end; $$;
drop trigger if exists touch_commune_vote_ballots_updated_at on public.commune_vote_ballots;
create trigger touch_commune_vote_ballots_updated_at before update on public.commune_vote_ballots for each row execute function public.touch_commune_vote_ballots_updated_at();
...
-- Governed sandbox schema snapshot.

-- Canonical history remains in the additive migrations dated 2026-06-24, 2026-06-25, and 2026-07-13.
...
-- Governed Coding Cornucopia sandbox reservations and finalization.
    ...
    return p_code_document_id is null
      ...
      and exists (
        ...
        where snippet.id = v_source_id
          ...
          and (post.status = 'published' or snippet.author_user_id = p_actor or v_reviewer)
      );
