...
import { executeRunner, sanitizeRunnerResult, unavailableRunnerResult } from "./_shared/runner.ts";
...
export type RunDependencies = {
  ...
  start(auth: AuthenticatedRequest, runId: string, clientRequestId: string, env: Env): Promise<void>;
  execute(env: Env, runId: string, source: AuthorizedSource): Promise<RunnerResult>;
...
function requiredFinalizerToken(env: Env): string {
  ...
  if (!token || token.length < 32 || token.length > 512) throw new PublicHttpError(503, "sandbox_misconfigured");
  return token;
}

function replayResult(reservation: Reservation, source: AuthorizedSource): PublicSandboxResult {
  const result = reservation.result;
  if (!reservation.runId || !result) throw new PublicHttpError(409, "idempotent_request_pending", 4);
  const sanitized = sanitizeRunnerResult(result, source);
  return {
    ...sanitized,
...
export async function handleSandboxRun(
  ...
  try {
    ...
    if (reservation.idempotentReplay) return jsonResponse(replayResult(reservation, source));
    if (!reservation.runId) throw new PublicHttpError(503, "reservation_unavailable");

    await dependencies.start(auth, reservation.runId, parsedRequest.clientRequestId, env);

    let runnerResult: RunnerResult;
    try {
      runnerResult = await dependencies.execute(env, reservation.runId, source);
    } catch (error) {
