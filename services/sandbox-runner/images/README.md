...
Production does not use mutable upstream language images directly. Each Containerfile takes an upstream base reference that has already been pinned by digest, then removes shells, BusyBox userland, package managers, package installers, and root caches. The final image runs as numeric user `65534:65534` and is invoked only with the runner's fixed argv.
